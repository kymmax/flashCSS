...
function flashCSS( PARA ) {
	...
	var _para_spacing = {
		...
		":bind": function(dom, name){
			if(dom.observe) return;
			dom.observe = true;
			// 
			var state = name.includes("tog");
			var bind = state ? name.split("-")[2] : name.split("-")[1];
			// 
			dom.addEventListener("click",function(){
				console.log("BIND!")
				document.querySelectorAll( ".bind-" + bind ).forEach(function(i){

					state ? i.classList.toggle("active") : i.classList.add("active");
				})
			})
		},
	};
		...
		document.querySelectorAll("[class]").forEach(function (index) {
			...
			_class_string.forEach(function (i) {
				Object.keys(_para_spacing).forEach(function (spacing) {
					// for JS
					if(spacing.includes(":") && i.includes(spacing)){
						_para_spacing[spacing](index, i);
						return;
					}
					// for CSS
