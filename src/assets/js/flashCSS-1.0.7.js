				...
				Object.keys(_para_spacing).forEach(function (spacing) {
					...
					Object.keys(_para_media).forEach(function (media) {
						...
						if (i.indexOf(_check_string) == 0 && !_value_class.includes("-") && !_class_record_skip) {
							...
							_value_class = _value_class.replace(_para_symbol_reg,function(match, offset, string){
								return (offset > 0 ? '' : '\\') + match;
							});  
							console.log("_value_class: ", _value_class);
							
							// Value Replace
							_value = _value.replace(_para_symbol_reg, function($0, $1) {
								return _para_symbol[$1];
							});
							if(_value.includes("#")){
								var _hex = _value.split("#")[1].substring(0,6);
								var _rgb = _self.hexToRgb("#" + _hex);
								_value = _value.replace("#" + _hex, _rgb.r + "," + _rgb.g + "," + _rgb.b )
							...
							// console.log(_class_string_merge);
